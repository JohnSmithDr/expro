...
/**
 * Converts string value into array.
 *
 * @param x - should be a string value
 * @param {string} [separator]
 * @returns {Array}
 * @private
 */
const _toArray = (x, separator) => {
  let type = typeof x;
  if (type === 'string' && x) return x.split(separator);
...
/**
 * Create array conversion middleware.
 *
 * @example
 *
 *   expro.query.prop('foo').toArray();        // default separate by ','
 *   expro.query.prop('bar').toArray('_');    // separate by '_'
 *
 * @param {object} [separator] - default to ','
 * @returns {function}
 */
function toArray(separator) {
  return convert.call(this, x => _toArray(x, separator || ','));
}

module.exports = { convert, toInt, toNumber, toBool, toArray };